    ...
    <tbody>
        @foreach($lunas as $l)
            <tr id="tr_{{$l->no_bukti}}">
                ...
                <td style="text-align: center;"><a class='btn {{$l->lunas == "Belum Lunas" ? "btn-danger" : "btn-success"}} btn-update-bayar' href="{{route('UpdateBayarJual',$l->no_bukti)}}" @if($l->lunas == "Lunas") style="pointer-events: none; cursor: default;" @endif>{{$l->lunas}}</a></td>
            </tr>
        @endforeach
    </tbody>
  </table>
