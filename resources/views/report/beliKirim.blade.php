    ...
    <tbody>
        @foreach($kirim as $k)
            <tr id="tr_{{$k->no_bukti}}">
                ...
                <td><a class='btn btn-info' href="{{route('BeliDetail',$k->no_bukti)}}">Details</a></td>
                <td><a class='btn {{$k->status == "Belum Terkirim" ? "btn-danger" : "btn-success"}} btn-update-kirim' href="{{route('UpdateKirim',$k->no_bukti)}}" @if($k->status == "Sudah Terkirim") style="pointer-events: none; cursor: default;" @endif>{{$k->status}}</a></td>
            </tr>
        @endforeach
    </tbody>
  </table>
